#!/bin/bash
echo " 限制CentOS6.X系列 初始化安装请确保网络通畅DNS解析正常......" && sleep 2s 
file1="/opt/apache-tomcat-8.5.23.tar.gz"
...
mysqlpasswd="Nu2.&#HkSr18^%"
echo -e "请把下列安装包放到/opt目录下 \n$file1\n$file2\n$file3\n$file4\n$file5\n$file6 " $$ sleep 2s
if [ -f "$file1" ];then
	echo "文件 $file1 存在"
else
	echo "文件 $file1 不存在将自动下载" && wget -P /opt/  https://linux-1254084810.cos.ap-chengdu.myqcloud.com/apache-tomcat-8.5.23.tar.gz
fi
if [ -f "$file2" ];then
	echo " 文件 $file2 存在 "
else
...
rpm -ivh mysql-community-release-el6-5.noarch.rpm -y
yum install mysql-community-server -y
service mysqld start
sleep 2s && echo "正在启动mysql"
/usr/bin/mysqladmin -u root password $mysqlpasswd
sleep 2s && echo "您的mysql密码是:$mysqlpasswd"
echo "Your Mysql PASSWord is :$mysqlpasswd" >> mysql_PASSWD.txt
cd /opt/
...
cd /opt/
mkdir -p /opt/java
tar -zxvf $file1 -C /opt/java
tar -zxvf $file3 -C /opt/java
\cp -R /opt/java/apache-tomcat-8.5.23/ /opt/java/apache-tomcat-env2
echo '''
...
export TOMCAT_HOME=/opt/Java/apache-tomcat-8.5.23
export CATALINA_HOME=/opt/Java/apache-tomcat-8.5.23
export CLASSPATH=$CLASSPATH:$CATALINA_HOME/common/lib
export CATALINA_BASE=/opt/Java/apache-tomcat-8.5.23
''' >> /etc/profile
...
sed -i "s/apache-tomcat-8.5.23/apache-tomcat-env2/g"  /etc/init.d/env2
...
mkdir /opt/soft
mv /opt/*.gz /opt/*.rpm/opt/openssl-1.1.0g/ /opt/ngx_cache_purge-2.3/ nginx-1.13.6/ /opt/soft
echo '''
#!/bin/sh 
# 
...
#########################################################
###            ADD_SSH_IP   #####
#-A INPUT -s 124.6.185.179/32 -m state --state NEW -m tcp -p tcp --dport 48456 -j ACCEPT
...
-A INPUT -m state --state NEW -m tcp -p tcp --dport 48456 -j ACCEPT
#################################################################
##            ADD_ACCESS_IP  ###
#Local_address
...
##         HZ_HOME                ###
-A INPUT -s 180.191.155.80 -j ACCEPT
##         oFFICE                ###
-A INPUT -s 124.6.185.179 -j ACCEPT
##########################ICMP_ACCEPT############################
-A INPUT -s 124.6.185.179/29  -p icmp -j ACCEPT
-A INPUT -s 180.191.155.80/29  -p icmp -j ACCEPT
-A INPUT -s 180.191.155.80  -p icmp -j ACCEPT
